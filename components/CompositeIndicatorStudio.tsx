...
export function CompositeIndicatorStudio({ lang, masterCue }: { lang: Lang; masterCue: string }) {
  ...
  const apiComponents: Record<string, string> = { 'leaf count': 'leaf_count', 'crown diameter': 'crown_diameter_mm', 'plant height': 'plant_height_cm', 'unopened flower count': 'unopened_flower_count', 'open flower count': 'open_flower_count', 'fruit set count': 'fruit_set_count' };
  const saveSpecification = async () => {
    setSpecState('saving');
    try { const response = await fetch('/api/cmaa/specification-drafts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ caseId: 'B2-20251211', candidateId, selectedComponents: selectedComponents.map((component) => apiComponents[component] || component), masterCue, boundaryNote }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.code || payload.detail || 'bridge_request_failed'); setSpecError(''); setSpecState('saved'); } catch (error) { setSpecError(error instanceof Error ? error.message : 'bridge_request_failed'); setSpecState('error'); }
  };

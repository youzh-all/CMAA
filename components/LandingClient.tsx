...
export function LandingClient() {
  ...
  return <main className="landing">
    ...
    <section className="landing-hero"><img className="scene-background" src="/landing/greenhouse-background.png" alt="Strawberry greenhouse illustration" /><div className="scene-wash"></div><img className="scene-master" src="/landing/master-grower.png" alt="Master grower" /><img className="scene-agent" src="/landing/apprentice-agent.png" alt="Apprentice agent" /><div className="hero-copy"><p>{t.eyebrow}</p><h1>{t.title}</h1><p className="hero-lead">{t.lead}</p><div className="hero-actions"><Link href="/workspace" className="primary-cta">{t.primary} <b>→</b></Link><a href="#workflow" className="secondary-cta">{t.secondary}</a></div><div className="trust-row"><span>● {t.trustA}</span><span>● {t.trustB}</span><span>● {t.trustC}</span></div></div></section>
